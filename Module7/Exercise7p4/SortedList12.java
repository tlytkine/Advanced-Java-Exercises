// File: SortedList12.java (Module 7)
//
// Author: Rahul Simha
...
class LinkedList implements MinMax {
    ...
    {	
    	enumPtr = front;
    	ListItem min = enumPtr;
	    while (hasMoreElements()) {
	    	ComparableObject temp = enumPtr.data;
		if (min.data.compare(temp) < 0){
			min.data = enumPtr.data;
		}
		temp = (ComparableObject) nextElement();
	    }
	    Object minE = min.data;
	    return minE;
    }
    ...
    {
 		enumPtr = front;
    	ListItem max = enumPtr;
	    while (hasMoreElements()) {
	    	ComparableObject temp = enumPtr.data;
		if (max.data.compare(temp) > 0) {
			max.data = enumPtr.data;
		}
		temp = (ComparableObject) nextElement();
	    }
	    Object maxE = max.data;
	    return maxE;
    }
